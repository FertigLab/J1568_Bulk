---
title: "Preprocessing and Comaprison of Sample Filtering -- OS Differential Expression"
...
print("OS Samp Numbers-- >2y (greater) <2y (less)")
table(sampAnnotFilter$Cell_Type,
      sampAnnotFilter$OS)
```
...
DEByCellType <- function(CellType) {
  sampByCell <- sampAnnotFilter[sampAnnotFilter$Cell_Type == CellType & !is.na(sampAnnotFilter$OS),]
  ...
  dds <- DESeqDataSetFromMatrix(countData =
                              ...
                              design=~OS+batch)
  dds <- DESeq(dds)
  res <- results(dds, name="OS_less_vs_greater")
  res <- lfcShrink(dds, coef="OS_less_vs_greater", 
                   res=res)
...
EnhancedVolcano(CD4DE, lab=row.names(CD4DE), 
                ...
                pCutoff = 0.05, selectLab = c("DUSP10", "FBXL20", "FBXO33", "SKP2", "CXCR2", "TLR9"), title="CD4 DE Results by Outcome", drawConnectors = T,maxoverlapsConnectors=100, labSize = 3)
...
write.csv(TopDE, "CD4_TopDE_Outcome.csv")

sampCD4 <- sampAnnotFilter[sampAnnotFilter$Cell_Type == 'CD4' & !is.na(sampAnnotFilter$OS),]

dfAnnot = data.frame(OS=sampCD4[,'OS'],
...
fwrite((CD4Res$gs$KEGG), file="CD4_KEGG_Outcome.csv")
...
ggplot(CD4Res$gs$HALLMARK, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  title="Hallmark pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD4ResHALLMARK)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) Hallmark pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
...
ggplot(CD4Res$gs$KEGG, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="KEGG pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD4ResKEGG)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) KEGG pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
...
fwrite((CD4Res$gs$REACTOME), file="CD4_REACTOME_Outcome.csv")
...
ggplot(CD4Res$gs$REACTOME, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="REACTOME pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD4ResREACTOME)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) REACTOME pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
```
...
fwrite((CD4Res$gs$GOBP), file="CD4_GOBP_Outcome.csv")
...
ggplot(CD4Res$gs$GOBP, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="GOBP pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD4ResGOBP)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) GOBP pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
```
...
fwrite((CD4Res$gs$GOMF), file="CD4_GOMF_Outcome.csv")
...
ggplot(CD4Res$gs$GOMF, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="GOMF pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD4ResGOMF)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) GOMF pathways Enrichment Score from Survival in CD4 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
```
...
EnhancedVolcano(CD8DE, lab=row.names(CD8DE), 
                x='log2FoldChange', y='padj', FCcutoff = 0.5,
                pCutoff = 0.05, selectLab = c("MYB", "ATP5F1A", "ATP6V1D", "ATP6V1E2", "RNF11", "RNF145", "USP1", "FBXO25", "USP33", "USP4", "OUTUD7B", "RIPK1", "TAB3", "TNFAIP3", "TNFAIP1", "CEACAM8", "CSF1R", "HLA-K"), title = "CD8 DE Results by Outcome", drawConnectors = T,maxoverlapsConnectors=100, labSize = 3)
...
write.csv(TopDE, "CD8_TopDE_Outcome.csv")
sampCD8 <- sampAnnotFilter[sampAnnotFilter$Cell_Type == 'CD8' & !is.na(sampAnnotFilter$OS),]

dfAnnot = data.frame(OS=sampCD8[,'OS'],
...
fwrite((CD8Res$gs$HALLMARK), file="CD8_HALLMARK_Outcome.csv")
...
ggplot(CD8Res$gs$HALLMARK, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Hallmark pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD8ResHALLMARK)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) Hallmark pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
```
...
fwrite((CD8Res$gs$KEGG), file="CD8_KEGG_Outcome.csv")
...
ggplot(CD8Res$gs$KEGG, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="KEGG pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD8ResKEGG)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) KEGG pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
...
fwrite((CD8Res$gs$REACTOME), file="CD8_REACTOME_Outcome.csv")
...
ggplot(CD8Res$gs$REACTOME, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="REACTOME pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD8ResREACTOME)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) REACTOME pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
...
fwrite((CD8Res$gs$GOBP), file="CD8_GOBP_Outcome.csv")
...
ggplot(CD8Res$gs$GOBP, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="GOBP pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD8ResGOBP)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) GOBP pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
```
...
fwrite((CD8Res$gs$GOMF), file="CD8_GOMF_Outcome.csv")
...
ggplot(CD8Res$gs$GOMF, aes(reorder(pathway, NES), NES, fill = adjPvalue)) +
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="GOMF pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
...
if (nrow(Sig_CD8ResGOMF)==0){ print("No Significant pathways with pAdj < 0.05"
  ...
  coord_flip() +
  labs(x="Pathway", y="Normalized Enrichment Score",
  title="Significant (pAdj<0.05) GOMF pathways Enrichment Score from Survival in CD8 Cells") + theme(plot.title.position = "plot", plot.title = element_text(size=8, face="bold"))
}
```
...
treatments$Survival <- sampAnnotFilter$OS
...
#### Ttest by OS
```{r}
datatable(CIBERSORTx_Res_CellTypes_CD4[c(1:22,24)] %>% 
  gather(key = variable, value = value, -Survival) %>% 
  group_by(Survival, variable) %>% 
  summarise(value = list(value)) %>% 
  spread(Survival, value) %>% 
  group_by(variable) %>% 
  mutate(p_value = t.test(unlist(greater), unlist(less))$p.value,
         t_value = t.test(unlist(greater), unlist(less))$statistic))

pdf(file="CD4_SigCibersort_Outcome.pdf")
ggplot(CIBERSORTx_Res_CellTypes_CD4, aes(x=Survival, y=Mast.cells.activated))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD4 Cell Samples")
ggplot(CIBERSORTx_Res_CellTypes_CD4, aes(x=Survival, y=Macrophages.M2))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD4 Cell Samples")
ggplot(CIBERSORTx_Res_CellTypes_CD4, aes(x=Survival, y=Macrophages.M0))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD4 Cell Samples")
dev.off()
```
...
#### Ttest by OS
```{r}
datatable(CIBERSORTx_Res_CellTypes_CD8[c(1:22,24)] %>% 
  gather(key = variable, value = value, -Survival) %>% 
  group_by(Survival, variable) %>% 
  summarise(value = list(value)) %>% 
  spread(Survival, value) %>% 
...
pdf(file="CD8_SigCibersort_Outcome.pdf")
ggplot(CIBERSORTx_Res_CellTypes_CD8, aes(x=Survival, y=NK.cells.activated))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD8 Cell Samples")
ggplot(CIBERSORTx_Res_CellTypes_CD8, aes(x=Survival, y=Eosinophils))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD8 Cell Samples")
ggplot(CIBERSORTx_Res_CellTypes_CD8, aes(x=Survival, y=Plasma.cells))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD8 Cell Samples")
ggplot(CIBERSORTx_Res_CellTypes_CD8, aes(x=Survival, y=Macrophages.M2))+geom_boxplot()+geom_point(aes(col=Treatments, shape=Survival))+stat_compare_means(comparisons = list(c("greater","less")),method="t.test") + ggtitle("Boxplot of Cibersort Cell Fractions of CD8 Cell Samples")
dev.off()
```
...
Neoantigen_sampInfo = Neoantigen_sampInfo[Neoantigen_sampInfo$OS %in% c("greater","less"),]
```
...
ggplot(Neoantigen_plotdf, aes(x=OS, y=Total_mutationsite_valid)) + geom_boxplot() + geom_point(aes(col=Treatment,shape=OS)) + stat_compare_means(method = "t.test") + ggtitle("Boxplot of Total_mutationsite_valid by OS")
